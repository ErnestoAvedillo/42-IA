...
class Conv2D(Activation):
    def __init__(self, **kwargs):
        ...
        self.weights = np.random.randn(self.filters, self.kernel_size,
                                       self.kernel_size)
    ...
    def set_model(self, **kwargs):
        keys = ["weights", "bias", "data_shape", "kernel_size", "filters",
                "stride", "padding"]
        for key in keys:
    ...
    def calculate_delta_on_input(self):
        ...
        for i in range(self.delta.shape[0]):
            for j in range(self.delta.shape[1]):
                output[i, j] = signal.convolve2d(self.delta[i, j],
                                                 weights_rot[j], mode="full")
        return output
    ...
    def forward_calculation(self, X):
        ...
        for i in range(self.n_samples):
            ...
            for l in range(0, self.filters):
                aux = signal.convolve2d(X[i, l, :, :], self.weights[l, :, :],
                                        mode="valid") + self.bias
                outputs1.append(aux)
    ...
    def backward_calculation(self, next_layer):
        ...
        for i in range(self.delta.shape[0]):
            for j in range(self.delta.shape[1]):
                self.delta_weights[j] = signal.correlate2d(
                                                self.input[i, j, :, :],
                                                self.delta[i, j, :, :],
                                                "valid")
        velocity_weight, velocity_bias = self.optimizer.calculate_optimizer(
                                                self.delta_weights,
                                                self.delta_bias)
