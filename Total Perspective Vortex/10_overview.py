...
#/home/ernesto/mne_data/physionet/files/eegmmidb/1.0.0/S001
params = {'dataset_name': 'Brain',
         'archive_name': 'eeg-motor-movementimagery-dataset-1.0.0.zip',
         'url': "https://physionet.org/files/eegmmidb/1.0.0/",
         'folder_name': 'physionet'}

sample_data_folder = mne.datasets.fetch_dataset(dataset_params=params)
sample_data_raw_file = (
    sample_data_folder / "files" / "eegmmidb" / "1.0.0" / "S001" / "S001R01.edf"
)
...
raw.filter(1, 40)  # band-pass filter the data
raw.plot(duration=5, n_channels=30)
# %%
# Preprocessing
# ^^^^^^^^^^^^^
