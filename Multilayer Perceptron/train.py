...
for k in range(5 , 10):
    for j in range(10,20):
        ...
        #curr_accuracy =network[i].train(X_train, Y_train, X_test, Y_test, epochs=100,  learning_rate=0.01)
        curr_accuracy =network[i].train(X_train, Y_train, X_test, Y_test, epochs=100, batch_size=64, learning_rate=0.01)
...
network.train(X_train, Y_train, X_test, Y_test, epochs=100, learning_rate=0.001)
y_predicted = network.predict(X_test)
network.save_model(model_file)
metrics = network.evaluate_prediction(Y_test, y_predicted)
