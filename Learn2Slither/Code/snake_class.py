...
class Snake(pg.sprite.Sprite, EnvSnake):
    def __init__(self, x, y, Nr_cells=[10, 10], modelname=None):
        ...
        if modelname is None or not os.path.exists(modelname):
            print(f"Model for auto-play not added or invalid. No autoplay will be available.")
            ...
            if self.menu_active:
                ...
                if event.type == pg.MOUSEBUTTONDOWN:
                    if self.manual_button.collidepoint(pg.mouse.get_pos()):
                        self.run()
                        return
                    elif self.show_autoplay and self.auto_button.collidepoint(pg.mouse.get_pos()):
                        self._auto_play()
                        return
